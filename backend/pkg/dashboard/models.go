...
type ReflectorData struct {
	...
	Nodes []struct {
		...
		ListenOnly    string `xml:"LISTENONLY" json:"ListenOnly"`
		ConnectTime   string `xml:"CONNECTTIME" json:"connecttime"`
		LastHeardTime string `xml:"LASTHEARDTIME" json:"lastheardtime"`
...
func (r *Reflector) GetStations() map[int][]string {
	...
	for i, station := range data.Stations {
		c := strings.Fields(station.Source)
		if len(c) < 2 {
			c = append(c, " ")
		}
		callsign, module := c[0], c[1]
		stations[i] = append(stations[i], callsign, module, station.Destination, station.Mode, station.Via, station.OnModule, station.LastHeardTime)
	}
	return stations
}
